...
import { RotateCcw, Search, Filter, Eye, Loader2 } from 'lucide-react';
    ...
    if (error) {
        return (
            <div className="text-center p-10">
                <p className="text-red-500 font-bold mb-4">Error loading refund data: {error?.data?.message || error?.error || "Unknown Error"}</p>
            </div>
        );
    }

    // Filter refunded orders
    let refundedOrders = orders ? orders.filter(o => o.orderStatus === 'refund') : [];

    if (searchTerm) {
        refundedOrders = refundedOrders.filter(o =>
        ...
        <div className="max-w-[1600px] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="mb-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-slate-500 mt-2 font-medium">Manage and track customer refunds</p>
                </div>
            </div>
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-slate-50/80 sticky top-0 z-10 backdrop-blur-sm">
                            <tr>
                                ...
                                <th className="px-6 py-5 text-[11px] font-black text-slate-400 uppercase tracking-widest text-right whitespace-nowrap">Amount</th>
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {refundedOrders.length > 0 ? refundedOrders.map((order) => (
                                <tr key={order._id} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="px-6 py-5 text-center">
                                        <span className="text-sm font-medium text-slate-700">Customer Request</span>
                                    </td>
                                    <td className="px-6 py-5 text-right">
                                        <span className="text-sm font-black text-slate-900">₹{order.totalPrice.toLocaleString()}</span>
                                    </td>
                                    <td className="px-6 py-5 text-center">
                                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-50 text-rose-700 text-xs font-black uppercase tracking-wider">
